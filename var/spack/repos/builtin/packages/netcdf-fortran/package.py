...
class NetcdfFortran(AutotoolsPackage):
    ...
    def libs(self):
        ...
        libs = find_libraries(libraries, root=self.prefix,
        ...
        raise spack.error.NoLibrariesError(
            msg.format("shared" if shared else "static",
                       self.spec.name, self.spec.prefix)
        )
    ...
    def patch_libtool(self):
        ...
        if "%aocc" in self.spec:
            ...
            filter_file(r'^wl=""$', 'wl="{0}"'.format(
                self.compiler.linker_arg), "libtool")
            ...
            # a solution.
            filter_file(
                r'^pic_flag=""$', 'pic_flag=" {0}"'.format(
                    self.compiler.fc_pic_flag), "libtool"
            )
